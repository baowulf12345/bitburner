...
async function getHudData(ns, bitNode, dictSourceFiles, options) {
	...
	// collect and display the delta of cash from the previous second for a real-time view of cashflow if the incomedelta script is running.
	if (ns.fileExists("lib/incomedata.txt") && ns.isRunning("lib/incomedelta.js","home")) {
		var incomeData = JSON.parse(ns.read("lib/incomedata.txt"));
		var highest = 0;
		var delta = 0;
		for (let entry of Object.keys(incomeData)) {
			if (Number(entry) > Number(highest)) {
				highest = Number(entry);
				delta = Number(incomeData[entry]["delta"]);
			}
		}
		hudData.push(["Inc Δ", true, formatNumberShort(delta, 3, 2) + '/sec', "Income delta from previous second."]);
	}
