...
export async function main(ns) {
	...
	var threads = file.weakeninfo.weakthreads;
	var delay = rundata.weakendelay;
	var executingserver = ns.getHostname();

	await ns.sleep(delay);
	ns.exec(settings.scripts.weakenscript, executingserver, threads, rundata.target);
	//ns.tprint("executing sched_weaken");
}